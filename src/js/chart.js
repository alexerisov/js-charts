...
import { isOver, toDate, circle, line,
    boundaries, css, toCoords, computeXRatio, computeYRatio } from './utils'
...
export function chart(root, data) {
    ...
    const slider = sliderChart(root.querySelector(
        '[data-el="slider"]'), data, DPI_WIDTH)
    let prevMax
    let prevMin
    let raf
    ...
    function paint() {
        ...
        // const min = getMin(yMin)


        const yRatio = computeYRatio(VIEW_HEIGHT, max, yMin)
        const xRatio = computeXRatio(VIEW_WIDTH, columns[0].length)

        const translate = translateX(data.columns[0].length,
            xRatio, proxy.pos[0])

        const yData = data.columns.filter(col => data.types[col[0]]
            === 'line')
        const xData = data.columns.filter(col => data.types[col[0]]
            !== 'line')[0]
